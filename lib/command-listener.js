...
app.get('/rules', function (req, res) {
   ...
   res.send(rules).end();
   logger.debug('got request for current rules', rules);
});
...
app.put('/rule', function (req, res) {
   ...
   res.type('application/json').status(201).send(rules).end();
   logger.debug('new rule added', req.body.rule.options);
});
